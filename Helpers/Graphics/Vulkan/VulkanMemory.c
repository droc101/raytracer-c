...
bool AllocateLocalMemory()
{
    for (uint32_t i = 0; i < physicalDevice.memoryProperties.memoryTypeCount; i++)
    {
        if (memoryPools.localMemory.memoryTypeBits & 1 << i &&
            (physicalDevice.memoryProperties.memoryTypes[i].propertyFlags & memoryPools.localMemory.type) == memoryPools.localMemory.
            type)
        {
...
bool AllocateSharedMemory()
{
    for (uint32_t i = 0; i < physicalDevice.memoryProperties.memoryTypeCount; i++)
    {
        if (memoryPools.sharedMemory.memoryTypeBits & 1 << i &&
            (physicalDevice.memoryProperties.memoryTypes[i].propertyFlags & memoryPools.sharedMemory.type) == memoryPools.sharedMemory.
            type)
        {
            const VkMemoryAllocateInfo allocInfo = {
                VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
